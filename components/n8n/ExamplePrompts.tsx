...
export function ExamplePrompts({ setPrompt }: { setPrompt: (prompt: string) => void }) {
  ...
  return (
    <div className="mb-12">
      <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-gray-200 mb-8">Not sure where to start?</h2>
      <div className="grid md:grid-cols-2 gap-4">
        {examplePrompts.map((example, index) => (
          <button
            key={index}
            onClick={() => setPrompt(example)}
            className="text-left p-4 rounded-lg border border-gray-200 hover:border-primary/50 hover:bg-primary/5 transition-all duration-300 group flex items-center dark:border-gray-700 dark:hover:bg-gray-800"
          >
            <p className="text-gray-700 group-hover:text-gray-900 flex-grow dark:text-gray-300 dark:group-hover:text-gray-100">{example}</p>
